...
export default function Footer() {
  return (
    <footer className="border-t border-gray-200 py-12 px-4 md:px-6 lg:px-8">
      <div className="max-w-[1400px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
          {/* Logo */}
          <div>
            <a href="/" className="font-bold text-2xl inline-block">
          ...
          <nav className="flex flex-wrap justify-center gap-6">
            {navItems.map((item) => (
              <a
                ...
                className="text-foreground/70 text-sm transition-opacity duration-300 hover:opacity-100 hover:text-foreground"
              >
                {item}
              </a>
            ))}
        ...
        <div className="mt-8 pt-8 border-t border-gray-200 text-center">
          <p className="text-sm text-foreground/50">
            © {new Date().getFullYear()} Ecello. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
